...
legend("topleft", legend = c("ARIMA", "DLM"), lty = 1, col = c("red", "blue"))
...
From the monthly plot, there is a yearly trend in the AMOC over the period. There was some sudden plunges in some few years when AMOC strength dropped to an unprecedented low, however, it quickly recovered to the yearly strength in the previous years.
...
In ACF, the significant spike at lag 1 might indicate a non-seasonal MA(1) component, and the significant spike at lag 12 suggests a seasonal MA(1) component. Consequently, we begin with an ARIMA(0,0,1)(0,1,1)[12] model, indicating a seasonal difference, and non-seasonal and seasonal MA(1) components. (By the same logic, if we analyse PACF, we can choose to start with AR(1) component in both non-seasonal and seasonal parts, as PACF has spikes at lag 1 and lag 12)
(https://otexts.com/fpp2/seasonal-arima.html)

```{r}
# Plot the SARIMA with seasonal difference
par(mfrow=c(1,3), mar=c(4,4,2,2))
plot(diff(amoc_m_ts, lag = 12), main = 'Seasonal difference')
acf(diff(amoc_m_ts, lag = 12), main = 'ACF')
pacf(diff(amoc_m_ts, lag = 12), main = 'PACF')
```
...
amoc_m_ts <- diff(amoc_m_ts, lag=12) 

model_1 <- Arima(amoc_m_ts, order=c(0,0,1), seasonal = c(0,1,1), include.drift = TRUE) 
par(mar = c(3, 3, 2, 2))
tsdiag(model_1)
```

The residuals look random. From the ACF plot of residuals, we can see that there are some considerable spikes at lag 8 and lag 11. This might suggest some non-seasonal components. 

```{r}
# Test some SARIMA model
# Add AR(1)
model_2 <- Arima(amoc_m_ts, order=c(0,0,1), seasonal = c(0,1,2), include.drift = TRUE) 
par(mar = c(3, 3, 2, 2))
tsdiag(model_2)

# Raise MA(2)
model_3 <- Arima(amoc_m_ts, order=c(1,0,2), seasonal = c(0,1,1), include.drift = TRUE) 
par(mar = c(3, 3, 2, 2))
tsdiag(model_3)
# more uniformity, thus, we can continue raise MA(3)

# Raise MA(3)
model_4 <- Arima(amoc_m_ts, order=c(1,0,3), seasonal = c(0,1,1), include.drift = TRUE) 
par(mar = c(3, 3, 2, 2))
tsdiag(model_4)

# Raise AR(2)
model_5 <- Arima(amoc_m_ts, order=c(2,0,3), seasonal = c(0,1,1), include.drift = TRUE) 
par(mar = c(3, 3, 2, 2))
tsdiag(model_5)
...
```{r}
model_2 <- Arima(amoc_m_ts, order=c(0,0,1), seasonal = c(5,1,3), include.drift = TRUE) 
par(mar = c(3, 3, 2, 2))
tsdiag(model_2)
...
```{r}
model_5 %>% forecast(h=12) %>% plot()

```

From the statistical results, SARIMA(1,0,2) is slightly better than all other models, but the difference is not significant. 

```{r}
# Predict with SARIMA
pred_12m <- forecast(model_ds_m1, 12)
pred_12m_values <- diffinv(pred_12m$mean, lag = 12)
pred_12m$mean<- pred_12m_values[13:24]
plot(pred_12m)
```
...
Compare the results of e) to your quarterly forecasts.
